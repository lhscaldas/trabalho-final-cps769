    ...
    system_prompt_3 = """
        ...
        {{
            ...
            SELECT 
                ...
                DATETIME(rtt_train.timestamp, 'unixepoch') as DataHora
        ...
        {{
            ...
            "query": "
            SELECT 
                AVG(rtt_train.rtt) as Avg_Latency, 
                DATETIME(rtt_train.timestamp, 'unixepoch') as DataHora
            FROM rtt_train
            JOIN (
            SELECT 
            ...
            WHERE EXISTS (
                ...
                WHERE b1.client = b2.client 
                AND b1.server = b2.server 
                AND ABS(b1.timestamp - b2.timestamp) <= 50
            )
            GROUP BY (b1.timestamp / 50)
            ) as bursts
            ON rtt_train.timestamp BETWEEN bursts.Burst_Start AND bursts.Burst_End
...
if __name__ == "__main__":
    question_1 = "Qual a média da taxa de bitrate para o cliente rj e servidor pi para cada rajada entre 8 e 9h do dia 07/06/2024?"
    get_response(question_1)
    question_2 = "Qual a latência das medições que coincidem com rajadas de bitrate para o cliente rj e servidor pi entre 8 e 9h do dia 07/06/2024?"
    get_response(question_2)